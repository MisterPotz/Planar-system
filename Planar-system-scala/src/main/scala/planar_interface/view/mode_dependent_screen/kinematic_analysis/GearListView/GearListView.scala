package planar_interface.view.mode_dependent_screen.kinematic_analysis.GearListView
...
import planar_interface.view.mode_dependent_screen.kinematic_analysis.GearView.{AbstractGearViewControllerFactory, GearView, GearViewController, GearViewControllerFactory, ViewFactory}
import planar_interface.view.mode_dependent_screen.kinematic_analysis.GearGroupView.{AbstractGearGroupOnlyViewControllerFactory, GearGroupOnlyViewController}
...
class GearListViewControllerFactory extends AbstractGearListViewControllerFactory{
  ...
  override def createView(): AnyRef= {
    ...
    val gearControllersList = gearsList.zipWithIndex.map((gearWheel) =>  {
      ...
      gearViewControllerFactory.setGearNumber(gearWheel._2+1)
