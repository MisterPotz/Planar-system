...
object B_AH_B_WheelCalculator extends WheelCalculator {
  ...
  override def synthesizeByWheelNumber(wheelNumberArgs: CommonMechanismCharacteristics.WheelNumberArgs): SynthesizedMechanisms = {
    ...
    //делаем массив по i-
    val variants = B_AH_B_WheelCalculator.findVariants(U = math.abs(wheelNumberArgs.targetU - 1), U_accuracy = wheelNumberArgs.accuracy)
    ...
    val pack = SynthesizedMechanisms(listBuff.toList, mechClassifier = B_AH_B)
    filterBySize(pack)
  }

  private def filterBySize(synthesizedMechanisms: SynthesizedMechanisms): SynthesizedMechanisms = {
    sortedByMin(minimalSize(synthesizedMechanisms))
  }
  override def minimalSize(synthesizedMechanisms: SynthesizedMechanisms): SynthesizedMechanisms = {
    ...
    val min_ : Int = gears(0).holder.z + gears(1).holder.z
    synthesizedMechanisms.copy(minimalSize = min_)
  }

  override def sortedByMin(synthesizedMechanisms: SynthesizedMechanisms): SynthesizedMechanisms = {
    val arr = synthesizedMechanisms.sorted_mechanisms
    val new_sorted = arr.sortBy(mech => {
      val s = mech.getGears
      s(0).holder.z + s(1).holder.z
    })
    synthesizedMechanisms.copy(sorted_mechanisms = new_sorted)
  }
}